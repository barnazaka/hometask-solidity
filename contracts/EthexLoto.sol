...
contract EthexLoto is Ownable {
    ...
    // Returns dynamic house edge percent based on number of marked cells:
    // 1 cell -> 12%, 2-3 cells -> 10%, 4-6 cells -> 8%
    ...
    function placeBet(bytes22 params) external payable {
        ...
        // First pass: read markedCount to determine dynamic house edge
        (coefficient, markedCount, holdAmount) = getHold(msg.value, bet);
        require(markedCount > 0, "Bet must have at least one marked cell");

        uint256 houseEdgePercent = getHouseEdge(markedCount);
        uint256 jackpotFee = msg.value * JACKPOT_PERCENT * PRECISION / 100 / PRECISION;
        uint256 houseEdgeFee = msg.value * houseEdgePercent * PRECISION / 100 / PRECISION;
        uint256 betAmount = msg.value - jackpotFee - houseEdgeFee;

        // Second pass: compute hold against net betAmount
